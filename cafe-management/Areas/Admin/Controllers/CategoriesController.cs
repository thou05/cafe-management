...
//            int pageSize = 30;
//            int pageNumber = page == null || page < 0 ? 1 : page.Value;
//            var listItem = _context.TbCategories
//                                    .AsNoTracking()
//                                    .OrderBy(x => x.Id)
...
//            var listItem = _context.TbCategories.AsNoTracking().Where(x => x.Name.ToLower().Contains(search)).OrderBy(x => x.Name).ToList();
//            PagedList<TbCategory> pagedListItem = new PagedList<TbCategory>(listItem, pageNumber, pageSize);
...
//            var product = _context.TbCategories.Where(x => x.Id == id).ToList();
...
//            _context.Remove(_context.TbCategories.Find(id));
//            _context.SaveChanges();
